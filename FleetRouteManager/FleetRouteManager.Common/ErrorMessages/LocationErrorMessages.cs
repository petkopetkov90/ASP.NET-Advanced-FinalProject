namespace FleetRouteManager.Common.ErrorMessages
{
    public static class LocationErrorMessages
    {
        ...
        public const string LocationPhoneNumberFormatMsg = "Phone number must contain only digits (0-9) and may optionally start with a '+'.<br>It may contain spaces ( ), hyphens (-) or periods (.) between groups of digits, but only one symbol is allowed in a row.";
